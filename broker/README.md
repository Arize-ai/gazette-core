...
- **`appendFlowControl`** (`append_flow_control.go`): Polices each Append stream against `MinAppendRate`
  ...
  time into `wait` vs `process` and is logged with the failure. The minimum is enforced as a byte
  budget sized by `MinAppendRateGrace` (`--broker.min-append-rate-grace`), which is only *two* 32KB
  chunks deep at the default rate: a bursty writer holding an append open across gaps needs the grace
  raised, since lowering `MinAppendRate` scales the budget and its decay together and so changes
  nothing. `max_gap_millis` in the log, not `idle_millis`, identifies the gap responsible.
