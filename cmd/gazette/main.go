...
var Config = new(struct {
	Broker struct {
		...
		DisableStoreHealthCheck bool          `long:"disable-store-health-check" env:"DISABLE_STORE_HEALTH_CHECK" description:"Disable the health check of fragment stores"`
...
func (cmdServe) Execute(args []string) error {
	...
	stores.DisableStoreHealthCheck = Config.Broker.DisableStoreHealthCheck
	log.WithFields(log.Fields{
		"disable-signed-urls":        stores.DisableSignedUrls,
		"disable-store-health-check": stores.DisableStoreHealthCheck,
	}).Warn("fragment store configuration")
